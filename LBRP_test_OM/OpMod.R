...
## DATA SECTION
setwd("~/Documents/CURRENT PROJECTS/OSU MPE Complexity/Models/")
...
Asam	<-	function(theta,yr,ct)
{
	with(as.list(theta),
	{
		...
		return(list(Bt=Bt, ut=ct/Bt[1:length(yr)]))
	})
}
...
if(!exists("S1")){
	...
	S1	<- list(MP1=CMP1.1,MP2=CMP2.1,MP3=CMP3.1,MP4=CMP4.1,MP5=CMP5.1,MP6=CMP6.1)
}
if(!exists("S2")){
	...
	S2	<- list(MP1=CMP1.2,MP2=CMP2.2,MP3=CMP3.2,MP4=CMP4.2,MP5=CMP5.2,MP6=CMP6.2)
}
...
{	#This function plot summary statistics for a given scenario
	...
	bxplts<-function(S)
	{
		...
		abline(v=6.5,lty=3); text(3,0,"Short term"); text(9,0,"Long term")
...
par(mfrow=c(6,2),mar=c(2,2,1,1),oma=c(4,3,2,4))
